...
export default function ProjectTemplate({ data }) {
  ...
  const projectIntroduction =
    data.contentfulProject.introduction.childMarkdownRemark
  ...
  return (
    <Layout>
      <Seo
        title={projectName}
        description={projectIntroduction.excerpt}
        imageURL={imageURL}
        pageURL={pageURL}
      />
      <div className="w-100 bg-light-gray">
        <div className="mh3 w-90-m w-80-l mw8 center-ns bg-light-gray">
          <h1 className="head-1-shadow f2 lh-title fw7 mv3 dark-gray">
            {projectName}
          </h1>
          <div>
            <h2 className="f3 lh-title tj fw7 mv4 bb b--black-40">
              {headOfIntroduction}
            </h2>
            <section
              key="project-template-introduction"
              dangerouslySetInnerHTML={{ __html: projectIntroduction.html }}
            />
            <section
              key="project-template-section"
              dangerouslySetInnerHTML={{ __html: projectSection.html }}
            />
          </div>
