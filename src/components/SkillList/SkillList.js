...
const SkillList = ({ slug, data, bgColor, isExpanded }) => {
  ...
  return (
    <section
      id={slug}
      className={`w-100 ${bgColor} pt3 pb5
    ...
    >
      ...
      >
        <div className="w-100 w-90-m w-80-l ph3 mw8 center">
          <h2 className="tc head-1-shadow f3 f2-ns lh-title fw7 mv4 dark-gray">
            {data.title}
          </h2>
          <section
            key="skill-list-content"
            dangerouslySetInnerHTML={{
              __html: data.content.childMarkdownRemark.html,
            }}
          />
          <section
            key="skill-list-cards-container"
            ...
            {data.complexData.map((skill, i) => {
              return (
                <article
                  ...
                  className=" bg-light-gray br3"
                >
                  <h2 className="w-100 mt2 tc f4 fw7">
                    <span key="skill-list-card-icon">
                    ...
                    {skill.items.map(item => {
                      return (
                        <li key={`skill-item-${item}`} className="mv1 w-100">
                          ...
                          <span
                            key="skill-list-item-name"
                            className="dib v-top w-90"
                          >
                            {item}
                          </span>
                        </li>
